...
!
!
 SUBROUTINE HARMO_Y_T (LMAX, LON, LAT, Y_T)
 USE SHTOOLS 
 IMPLICIT NONE 
...
!
!   The routine computes "y_t = d/dtheta y"
!**********************************************************
!
!
...
!
       			y_t(j) = dplm(j)*cmplx(cosdd(float(mj(j))*lon),sindd(float(mj(j))*lon)) 
!
    		enddo
!
!
